...
| FR-WINDOW-7 | **热键按键守卫**：触发快捷键中的那个按键（默认 `Space`）会在窗口拿到焦点后才被合成器补投递过来（实测比 `present()` 晚约 570 ms，且用户按住不放时会被系统自动重复成一串）。窗口必须在捕获阶段吞掉该键的按下事件，直到用户按下了别的键或超过 1.5 s 上限，否则空格会落进输入框。代价：窗口刚出现的极短时间内无法把该触发键本身当作第一个字符输入。 |
...
| 编号 | 规格 |
| --- | --- |
| FR-CLIP-1 | 只读取 `text/plain`。若剪贴板仅含图片、HTML、文件、富文本等，则按"无文本"处理，显示空输入框。 |
| FR-CLIP-2 | 每次唤起只读取一次，绝不监听剪贴板的持续变化。 |
| FR-CLIP-3 | 程序自身写入（复制译文）不会触发任何再翻译逻辑——因为不存在剪贴板监听，天然满足。 |
| FR-CLIP-4 | 读取到的文本直接填入输入框，用户可自由编辑。 |
...
| 编号 | 规格 |
| --- | --- |
| FR-INPUT-1 | 唤起后自动获得输入焦点，光标置于文本末尾。 |
| FR-INPUT-2 | 支持中文输入法、英文输入、粘贴、全选、编辑、光标移动。 |
| FR-INPUT-3 | 支持多行文本；输入框高度随内容自动增长，上限约 40% 屏高后内部滚动。 |
| FR-INPUT-4 | **输入法组合（preedit）期间，`Enter` 用于确认候选词，绝不能提交翻译**；只有输入法提交文字后用户再次按 `Enter` 才触发翻译。 |
