...
| 许可证 | MIT（版权人 2026 Pan Wenzheng、DeepSeek），正文见仓库根 `LICENSE`，`.deb` 内按 DEP-5 生成 `copyright` |
...
| FR-WINDOW-9 | **视觉层次**（M6 UI 打磨）：窗口本体透明，屏幕上可见的是一张卡片——圆角 12 px、1 px 描边（`alpha(currentColor, 0.10)`）、两层投影，四周留 20 px 给投影扩散，卡片内边距 14/16 px。无边框窗口在 GNOME 下不会获得系统阴影，若不自己画一层，卡片会与深色壁纸糊在一起。 |
...
| FR-INPUT-7 | **输入长度上限在输入环节生效**：一旦文本超过 DeepL 单次上限（128 KiB，按 UTF-8 字节计），立即就地截断到上限并提示「已达到上限（128 KiB），多余部分已被截断」，不让用户攒出一份注定发不出去的文本。裁剪按字节边界处理，不会把多字节字符截成半个。同时，文本超过约 4000 字符后关闭输入框的"高度自适应"改为固定高度内部滚动——GTK 为计算自然高度会布局整篇文本，超大文本下每次插入都会卡顿。 |
| FR-INPUT-8 | 输入区不画 GTK `GtkTextView` 默认的 `@theme_base_color` 底色（否则卡片里会浮出一块深色矩形）。垂直滚动条策略固定为 `EXTERNAL`：实测 `AUTOMATIC` 会让 `ScrolledWindow` **无论内容多高都固定多报 36 px 的自然高度**（一行中文也要 58 px），`EXTERNAL` 不占滚动条空间，高度贴合内容，滚轮/键盘滚动照常；只有切到"大文本固定高度"模式时才改用 `AUTOMATIC` 显示真正的滚动条。 |
...
| FR-RESULT-9 | 结果区固定带标题行，用于拉开原文与译文的层级：`译文` / `翻译中`（右侧跟 16 px spinner）/ `错误`，标题用 caption + dim 样式。 |
| FR-RESULT-10 | 底部提示行按当前状态生成，不做静态文案：已显示译文时才出现 `Ctrl+C 复制并关闭`（或关闭该设置时为 `Ctrl+C 复制译文`），否则只有 `Enter 翻译 · Shift+Enter 换行 · Esc 关闭`。 |
...
| FR-PRIV-2 | **禁止记录**：API Key、用户翻译文本、剪贴板内容。为防第三方库把请求细节（含 `Authorization` 头）写进日志，`httpx` / `httpcore` / `asyncio` 的 logger 被固定压到 `WARNING`，即使 `--verbose` 也不放开。 |
...
| 编号 | 指标 |
| --- | --- |
| PERF-1 | 常驻进程内，"快捷键按下 → 窗口可见"目标 < 100 ms（不含合成器动画）。实测：主实例侧首次呈现 63.5 ms（含创建窗口）、之后每次 0.4–0.8 ms。GSettings 后端的快捷键命令若直接启动进程，光"启动 Python 解释器 + 导入 GI"就要约 250 ms，因此启动器对 `--toggle`/无参数走 **D-Bus 快路径**（`org.freedesktop.Application.Activate`，实测 < 10 ms），仅在调用失败（主实例未运行）时才正常启动进程。 |
...
| T26 | 逐状态看窗口外观（空输入 / 已填输入 / 多行输入 / 译文 / 翻译中 / 错误） | 卡片圆角 + 投影清晰可见、输入行高度贴合内容、译文有 `译文` 标题、加载态有 spinner、错误为红色主文案 + 中文说明，底部提示随状态变化 |
...
| §15 / §17 | §15 用"Latin alphabet"判英文，§17 又把 `Bonjour` 列为"其他语言" | 遵循 §15：拉丁字母为主即判为英文（`source_lang=EN`）。**2026-09-12 用户确认保持现状**（主要场景为中英互译） | 两节自相矛盾：无法在不引入词典的前提下区分 Bonjour 与 hello。备选方案（较长的拉丁文本交给 DeepL 自动检测）已记录，V2 若需要多语种再启用 |
...
| 全文 | 只规定了"无标题栏、圆角、尽量小"，未涉及层次与排版 | 增补 FR-WINDOW-9 / FR-INPUT-8 / FR-RESULT-9 / FR-RESULT-10 | M6 UI 打磨：用户提供了空窗口、译文、设置三张实机截图，据此修掉"卡片与壁纸糊在一起""输入框内部一块深色矩形""译文与原文层级不足""加载态只有一行文字" |
